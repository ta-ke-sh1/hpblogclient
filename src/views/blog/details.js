...
export default function BlogDetails({ props }) {
    ...
    const titleOnMouseOver = (index) => {
        ...
        textShuffle(
            ...
            textContent,
            interval,
            300 / posterContent[index].name.length
        );
                        ...
                        {numbers.map((i, index) => {
                            return (
                                <div
                                    ...
                                    <h1
                                        onMouseEnter={() =>
                                            titleOnMouseOver(index)
                                        }
                                        className="blog-title noselect"
                                        id={"poster-title-" + index}
                                        data-original-text={
                                            posterContent[index].name
                                        }>
                                        {posterContent[
                                            index
                                        ].name.toLowerCase()}
                                    </h1>
                                    <span className="title-number med noselect">
                                        ({index + 1})
                                    </span>
                                </div>
                            );
                ...
                <div className="blog-image-container">
                    <div className="poster-holder">
                        {posters.map((poster, index) => (
                            <div key={"poster-" + index} className="blog-img">
                                {poster}
                            </div>
                        ))}
                    </div>
